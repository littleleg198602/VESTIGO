...
except Exception:  # tkinter nemusí být v některých instalacích Pythonu
    tk = None
...
def run_gui(default_input_dir: Path, default_output_dir: Path) -> None:
    if tk is None or ttk is None or filedialog is None or messagebox is None:
        raise RuntimeError("Tkinter není dostupný v tomto Pythonu")
...
def main() -> None:
    ...
    if args.gui or not args.cli:
        try:
            ...
            LOG.warning("GUI se nepodařilo spustit (%s), přepínám na CLI režim.", exc)

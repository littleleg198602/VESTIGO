...
RC_DEFINITIONS: dict[int, RCDefinition] = {
    1: _rc(1, "LIN vodič - povolené číslo dílu", "LIN wire - approved part number", "Kontrola ověřuje, že číslo dílu vodiče odpovídá povolenému seznamu pro LIN.", "This check verifies that the wire part number matches the approved LIN list.", ["Leitungsnummer", "Teilenummer der Leitung"], ["Teilenummer der Leitung", "Prüfung Parameter"], "Změnit číslo dílu vodiče na povolený typ pro LIN.", "Change the wire part number to an approved LIN type.", object_type_cz="Vodič", object_type_en="Wire", handler="rc1"),
    106: _rc(106, "Shoda barvy vodiče a signálu", "Wire and signal color consistency", "Kontrola porovnává skutečnou a požadovanou barvu vodiče pro daný signál.", "This check compares actual and required wire color for the given signal.", ["Signalname", "Signal"], ["IST-Farbe", "SOLL-Farbe", "Farben-Übereinstimmung"], "Ověřit a opravit shodu barvy vodiče s potenciálovou listinou.", "Verify and correct the wire color against the potential list.", object_type_cz="Vodič", object_type_en="Wire", handler="rc106"),
    110: _rc(110, "Kontrola délky speciálního vedení", "Special cable length consistency", "Kontrola vyhodnocuje rozdíl délek mezi vodiči ve speciálním vedení.", "This check evaluates length differences between wires in a special cable.", ["Sonderleitung", "Leitungen"], ["Sonderleitung", "Teilenummer", "Leitungen", "Potentiale", "Leitungslänge", "Ermittelte Längendifferenz (DMU)"], "Zkontrolovat délku vedení a upravit trasu nebo definici.", "Check the wire length and adjust routing or definition.", object_type_cz="Speciální vedení", object_type_en="Special cable", handler="rc110"),
    112: _rc(112, "Kontrola svazku", "Bundle consistency check", "Kontrola ověřuje konzistenci svazku vodičů.", "This check verifies wire bundle consistency.", ["Leitungsnummer"], ["Meldung", "Leitungsnummer"], "Opravit záznam svazku podle pravidla RC.", "Correct the bundle record according to the RC rule.", object_type_cz="Splice", object_type_en="Splice"),
    ...
    121: _rc(121, "Splice - shoda barev a potenciálu", "Splice - color and potential consistency", "Kontrola ověřuje konzistenci vodičů ve splice podle VOBES-ID.", "This check verifies splice wire consistency by VOBES-ID.", ["Splice", "VOBES-ID"], ["VOBES-ID", "Leitungsnummer", "Farbe Ist", "Farbe Soll", "Potential"], "Sjednotit barvy a potenciály vodičů uvnitř splice.", "Align wire colors and potentials within the splice.", object_type_cz="Splice", object_type_en="Splice", handler="rc121"),
    191: _rc(191, "Kontrola atributů vedení", "Wire attribute check", "Kontrola ověřuje povinné atributy vedení.", "This check verifies required wire attributes.", ["Leitungsnummer"], ["Meldung", "Leitungsnummer"], "Doplnit nebo opravit atributy vedení.", "Complete or correct wire attributes.", object_type_cz="Vodič", object_type_en="Wire"),
    ...
    325: _rc(325, "Kontrola zemnění", "Grounding consistency check", "Kontrola ověřuje konzistenci zemnění.", "This check verifies grounding consistency.", ["Potential"], ["Grundpotential", "Meldung"], "Opravit definici zemnění podle standardu.", "Correct grounding definition according to standard.", object_type_cz="Vodič", object_type_en="Wire"),
    338: _rc(338, "Kontrola napájení", "Power supply consistency check", "Kontrola ověřuje konzistenci napájecí větve.", "This check verifies power branch consistency.", ["Potential"], ["Meldung", "Potential"], "Opravit datový záznam napájení.", "Correct the power supply data record.", object_type_cz="Komponenta", object_type_en="Component"),
    ...
    1005: _rc(1005, "Kontrola varianty", "Variant consistency check", "Kontrola ověřuje konzistenci variantních dat.", "This check verifies variant data consistency.", ["Variant"], ["Variant", "Meldung"], "Opravit variantní nastavení dle pravidla.", "Correct variant setup according to rule.", object_type_cz="Varianta", object_type_en="Variant"),
    1007: _rc(1007, "Kontrola propojení", "Connection consistency check", "Kontrola ověřuje konzistenci propojení.", "This check verifies connection consistency.", ["Startpunkt", "Endpunkt"], ["Startpunkt", "Endpunkt", "Meldung"], "Opravit propojení mezi body podle pravidla.", "Correct the connection between points according to rule.", object_type_cz="Spoj", object_type_en="Connection"),
    3001: _rc(3001, "Kontrola délky trasy vedení", "Wire route length check", "Kontrola vyhodnocuje délku trasy vedení vůči očekávaným hodnotám.", "This check evaluates wire route length against expected values.", ["Potential"], ["Potential", "Startpunkt", "Leitungsnummer", "Leitungslänge [mm]", "Endpunkt", "Gesamtlänge", "Ergebnis Hinweis"], "Prověřit trasu vedení a upravit délku nebo definici trasy.", "Review wire path and adjust length or route definition.", object_type_cz="Vodič", object_type_en="Wire"),
